module Poseidon
  ...
  class BrokerPool
    ...
    def self.open(client_id, seed_brokers, socket_timeout_ms, ca_cert_path, &block)
      broker_pool = new(client_id, seed_brokers, socket_timeout_ms, ca_cert_path)
    ...
    def initialize(client_id, seed_brokers, socket_timeout_ms, ca_cert_path)
      @connections = {}
      @brokers = {}
      @client_id = client_id
    ...
    def fetch_metadata_from_broker(broker, topics)
      ...
      Connection.open(host, port, @client_id, @socket_timeout_ms, @ca_cert_path) do |connection|
    ...
    def new_connection(broker_id)
      ...
      Connection.new(info[:host], info[:port], @client_id, @socket_timeout_ms, @ca_cert_path)
    end
  end
end
